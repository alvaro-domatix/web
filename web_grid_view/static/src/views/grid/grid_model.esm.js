...
export class GridModel extends Model {
    ...
    _buildGrid(readGroupResult) {
        ...
        for (const group of readGroupResult) {
            ...
            if (this.hasSections) {
                ...
                sectionKey =
                    sectionVal instanceof Array ? sectionVal[0] : sectionVal || 0;
                sectionLabel = this._getGroupLabel(this.sectionFieldName, sectionVal);
            }
            ...
            for (const rfName of this.rowFieldNames) {
                ...
                const partLabel = this._getGroupLabel(rfName, rfVal);
                rowParts.push({
                    name: rfName,
                    key: partKey,
                    label: partLabel,
            ...
            row.cells[colId] = {
                ...
                value,
                domain: cellDomain,
                readonly: this.readonlyFieldName
                    ? Boolean(group[`${this.readonlyFieldName}:max`])
                    : false,
            };
        }
    ...
    /**
     * Formatted_read_group returns raw values for plain fields and
     * [id, display_name] pairs for relational ones. Selections need their
     * label looked up in the field definition, otherwise the technical value
     * ends up on screen.
     */
    _getGroupLabel(fieldName, value) {
        const field = this.fields?.[fieldName];
        if (field?.type === "selection") {
            const option = (field.selection || []).find((item) => item[0] === value);
